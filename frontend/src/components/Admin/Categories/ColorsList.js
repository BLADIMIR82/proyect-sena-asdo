...
export default function ColorsList() {
  const baseURL = "http://localhost:2030/api/v1/colors"
  //dispatch
  ...
  ///delete
  const deleteColorsHandler = async (id) => {
    try {
      await  axios.delete(`${baseURL}/${id}`)
      dispatch(fetchColorsAction());
      console.error(`exit deleting item: ${id}`)
  } catch (error) {
    console.error(`Error deleting item colors: ${id}`, error);
  }
  };
  return (
                    ...
                    {colors?.map((color) => (
                      <tr key={color?._id}>
                        ...
                        {/* delete icon */}
                         <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-left text-sm font-medium sm:pr-6">
                          <button
                           onClick={() => deleteColorsHandler(color._id)}
                            className="text-indigo-600 hover:text-indigo-900">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
